...
public:
    ...
    void place_char(int x, int y, char symbol)
    {
        move_to(x, y);
...
public:
    ...
    void initialize_grid(int level)
    {
        ...
        case 1:
            ...
            } while (key.isEqual(door));

            Pos player;
            do
            {
                player.set_pos(rand() % size, rand() % size);
            } while (player.isEqual(key) || player.isEqual(door));
        }
    }
